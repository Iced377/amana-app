...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {/* Default X close button, can be overridden by placing a DialogClose inside DialogHeader or DialogFooter */}
      <DialogPrimitive.Close 
        className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground"
        // Removed "hidden group-data-[custom-close=true]:block" to make it always visible unless explicitly overridden by a custom DialogClose
        >
        <X className="h-4 w-4" />
